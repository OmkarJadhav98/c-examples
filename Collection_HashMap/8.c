...
// Test if the hash map contains a mapping for the specified value
int containsValue(struct HashMap* map, char* value) {
    for (int i = 0; i < map->size; i++) {
        if (strcmp(map->data[i].value, value) == 0) {
...
int main() {
    ...
    // Test if the hash map contains a mapping for the specified values
    printf("1. Is value 'Green' exists?\n");
    printf("%s\n", containsValue(&hash_map, "Green") ? "Yes!" : "No!");

    printf("\n2. Is value 'orange' exists?\n");
    printf("%s\n", containsValue(&hash_map, "orange") ? "Yes!" : "No!");
